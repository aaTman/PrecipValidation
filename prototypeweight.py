...
latmid=(int(test1['lat'].shape[0]/2),int(test1['lat'].shape[1]/2))
lonmid=(int(test1['lon'].shape[0]/2),int(test1['lon'].shape[1]/2))
ll = LatLon(test1['lat'][latmid], test1['lon'][lonmid])
newlatlon=ll.destination(20000,45)
londiff=newlatlon.lon-test1['lon'][lonmid]
latdiff=newlatlon.lat-test1['lat'][latmid]
precipdata=test1['Total_precipitation_surface_1_Hour_Accumulation'][:]

for x in range(0,test1['lon'].shape[0]):
    for y in range(0,test1['lon'].shape[1]):
        
        indtestlon=np.where(np.logical_and(test1['lon']<=test1['lon'][x,y]+londiff,test1['lon']>=test1['lon'][x,y]-londiff))
        indcomblon=np.vstack((indtestlon[0][:],indtestlon[1][:])).T
        
        indtestlat=np.where(np.logical_and(test1['lat']<=test1['lat'][x,y]+latdiff,test1['lat']>=test1['lat'][x,y]-latdiff))
...
#        testones=np.random.randint(0,high=20,size=test1['lon'].shape)
...
#        c=ax1.pcolormesh(test1['lon'][:],test1['lat'][:],testones,transform=ccrs.PlateCarree(),cmap=plt.cm.Blues)
#        plt.colorbar(c)
...
#        ax1.add_feature(states, edgecolor='gray', zorder=3)
#        ax1.add_feature(ft.LAKES,alpha=0.5,facecolor='gray', zorder=3)
#        c=ax1.pcolormesh(test1['lon'][:],test1['lat'][:],test1['Total_precipitation_surface_1_Hour_Accumulation'][0].T,transform=ccrs.PlateCarree(),cmap=plt.cm.Blues)
#        plt.colorbar(c)
#        plt.show()
#        plt.close(fig)